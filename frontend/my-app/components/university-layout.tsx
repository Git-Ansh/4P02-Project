...
export function UniversityLayout({ navItems, children }: UniversityLayoutProps) {
  ...
  // Build CSS variable overrides from university theme (works in both light & dark)
  const themeStyle = React.useMemo(() => buildThemeStyle(theme), [theme]);

  // Also apply theme vars to <body> so dialog portals inherit the university theme
  React.useEffect(() => {
